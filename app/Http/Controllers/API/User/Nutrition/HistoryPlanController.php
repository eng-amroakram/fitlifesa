...
class HistoryPlanController extends Controller
{
    ...
    public function show($id)
    {
        ...
        if ((integer)\request()->query('type') == 2)
        {
            ...
            $plan = MealPlan::query()
                ...
                ->whereHas('historyMealTypes', function ($query) use($id){
                    $query->where('plan_id', $id)
                        ->where('status',0)
                        ->where('duration', (integer)\request()->query('duration'))
                        ->where('day_number','=',(integer)\request()->query('day_number'));
                })
        ...
        if((integer)\request()->query('type') == 1)
        {
