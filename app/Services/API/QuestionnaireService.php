...
class QuestionnaireService
{
    ...
    {
        try {
            UserBodyTracker::create(['user_id'=> auth()->guard('user-api')->user()->id,'weight'=> $request->customer_info['weight'], 'date'=> Carbon::now('Asia/Riyadh')->format('Y-m-d')]);
    ...
    private function addStarchesToServing($result,$protein, $carbsQ, $starches)
    {
        ...
        if ($constant["Starches"]["Protein"] + $result['total_protein'] < $protein
    ...
    {
        $constant  = $this->constant();
        if ($constant["Vegetables"]["Protein"] + $result['total_protein'] < $protein
    ...
    {
        $constant  = $this->constant();
        if ($constant["Meats"]["Protein"] + $result['total_protein'] < $protein
    ...
    private function addDairyToServing($result,$protein, $fatsQ,$dairy, $carbsQ)
    {
        ...
        if($constant["Dairy"]["Protein"] + $result['total_protein'] < $protein &&
